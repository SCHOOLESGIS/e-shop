...
@section('content')
<section class="site-banner jarallax min-height300" style="padding-top:40px; height:200px; background: linear-gradient(0deg, rgba(0,0,0,0.4), rgba(0,0,0,0.4)),url(images/hero-image.jpg) no-repeat; background-position: center; background-size: cover;">
    <div class="container">
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="container">
            <div class="widgets widget-menu">
                <div class="widget-search-bar">
                  <form action="{{ route("boutique.index") }}" method="GET" class="d-flex rounded">
                      <input type="text" class="border" placeholder="Rechercher une boutique" style="height: 50px;" name="search">
                    ...
                    @if (count($boutiques) > 0)
                        @foreach ($boutiques as $item)
                            <div class="product-item col-lg-3 col-md-6 col-sm-6" style="border-radius: 5px; height: 200px;  box-shadow: O O 10px 1px rgba(0, 0, 0, 1); margin-bottom: 60px;">
                                <a href="{{ route('boutique.show', [ 'boutique' => $item->slug]) }}">
                                    <div class="image-holder" style="border-radius: 5px; height: 200px; overfolw: hidden;  box-shadow: O O 10px 1px rgba(23, 22, 22, 0.1);">
                                        @if($item->logo != null)
                                            <img src="{{ asset('storage/'.$item->logo) }}" alt="Books" class="product-image border border-black" style="border-radius: 5px; height: 200px; object-fit: cover;">
                                        @else
                                            <img src="{{ asset('images/produits.jpg') }}" alt="Books" class="product-image border border-black" style="border-radius: 5px; height: 200px; object-fit: cover;">
                                        @endif
                                    </div>
                                </a>
