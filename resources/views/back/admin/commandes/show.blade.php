...
<main class="max-w-full mx-auto my-8 bg-white p-4 rounded shadow-md">
    ...
    <div class="flex justify-between items-center mt-10 px-8">
        <div class="">
            <a href="{{ route('admin.commande.index') }}" class="text-underline hover:text-amber-500">Historique des commandes</a>
        </div>
      <div class="text-right">
    ...
    <div class="">
        <div class="">
            @if ($commande->status == 'cancelled')
                <form action="{{ route('admin.commande.update', ['commande' => $commande->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="status" value="paid">
                    <button type="submit" class="w-full py-4 mt-6 rounded bg-black hover:bg-green-500 text-white hover:text-white text-lg flex gap-3 items-center justify-center" style="transition: all .2s .1s ease-in-out;"><span class="mt-2"><i class="fi fi-rr-usd-circle"></i></span> <span>Changer en <strong>payé</strong></span></button>
                </form>
                <form action="{{ route('admin.commande.update', ['commande' => $commande->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="status" value="shipped">
                    <button type="submit" class="w-full py-4 mt-6 rounded bg-black hover:bg-blue-500 text-white hover:text-white text-lg flex gap-3 items-center justify-center" style="transition: all .2s .1s ease-in-out;"><span class="mt-2"><i class="fi fi-rr-usd-circle"></i></span> <span>Changer en <strong>livré</strong></span></button>
                </form>
                <form action="{{ route('admin.commande.update', ['commande' => $commande->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="status" value="pending">
                    <button type="submit" class="w-full py-4 mt-6 rounded bg-black hover:bg-amber-500 text-white hover:text-white text-lg flex gap-3 items-center justify-center" style="transition: all .2s .1s ease-in-out;"><span class="mt-2"><i class="fi fi-rr-usd-circle"></i></span> <span>Changer en <strong>en attente</strong></span></button>
                </form>
            @elseif($commande->status == 'paid')
            <form action="{{ route('admin.commande.update', ['commande' => $commande->id]) }}" method="POST">
                @csrf
            ...
            <form action="{{ route('admin.commande.update', ['commande' => $commande->id]) }}" method="POST">
                ...
                <button type="submit" class="w-full py-4 mt-6 rounded bg-black hover:bg-red-500 text-white hover:text-white text-lg flex gap-3 items-center justify-center" style="transition: all .2s .1s ease-in-out;"><span class="mt-2"><i class="fi fi-rr-usd-circle"></i></span> <span>Changer en <strong>en annulée</strong></span></button>
            </form>
            @elseif($commande->status == 'pending')
                ...
                <form action="{{ route('admin.commande.update', ['commande' => $commande->id]) }}" method="POST">
                    ...
                    @method('PUT')
                    <input type="hidden" name="status" value="cancelled">
                    <button type="submit" class="w-full py-4 mt-6 rounded bg-black hover:bg-red-500 text-white hover:text-white text-lg flex gap-3 items-center justify-center" style="transition: all .2s .1s ease-in-out;"><span class="mt-2"><i class="fi fi-rr-usd-circle"></i></span> <span>Changer en <strong>annulée</strong></span></button>
                </form>
            @else
                <form action="{{ route('admin.commande.update', ['commande' => $commande->id]) }}" method="POST">
                    @csrf
