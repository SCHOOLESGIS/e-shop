...
@section('content')
    <div class="w-full h-full items-center justify-center flex flex-wrap flex-col p-2 rounded border shadow gap-2">
        <div class="lg:visible grow w-full h-[200px] rounded border flex items-center justify-center" style="background: linear-gradient(0deg, rgba(0,0,0,0.4), rgba(0,0,0,0.4)),url(/images/img-grid-2.jpg) no-repeat; background-position: center; background-size: cover;">
            <div class="w-full mb-4 flex justify-center">
                <small class="text-5xl capitalize font-bold text-white">Mettre à jour une boutique</small>
            </div>
        </div>
        <div class="w-full grow items-center justify-center flex flex-wrap flex-col">
            <form method="POST" action="{{ route('seller.boutique.update', ['boutique' => $boutique->slug]) }}" enctype="multipart/form-data" class="w-full min-h-[300px]">
                @csrf
                @method('PUT')
                <!-- Email Address -->
                <div class="flex w-full flex-wrap gap-3">
                    <div class="w-[300px] grow p-3 border rounded">
                        ...
                        <div class="mt-4">
                            ...
                            <input type="file" value="{{ $boutique->logo }}" name="logo" id="logo" class="block mt-1 w-full border border-slate-300 shadow-md rounded px-1 py-2">
