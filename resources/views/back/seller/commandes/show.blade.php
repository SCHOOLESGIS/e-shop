...
<main class="max-w-full mx-auto my-8 bg-white p-4 rounded shadow-md">
    ...
    <div class="flex justify-between items-center mt-10 px-8">
        <div class="">
            <a href="{{ route('seller.commande.index') }}" class="text-underline hover:text-amber-500">Historique des commandes</a>
        </div>
      <div class="text-right">
    ...
    <div class="">
        <div class="">
            @if ($commande->status == 'pending')
                <form action="{{ route('seller.commande.update', ['commande' => $commande->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
